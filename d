...
# Enable LTO
if [[ "$@" =~ "lto"* ]]; then
    echo "CONFIG_LTO_CLANG=y" >> arch/arm64/configs/"${DEF}"
    echo "CONFIG_THINLTO=n" >> arch/arm64/configs/"${DEF}"
    echo "CONFIG_TOOLS_SUPPORT_RELR=y" >> arch/arm64/configs/"${DEF}"
fi
...
# AOSP Build
os=aosp
make -j8 ${ARGS} 2>&1 | tee build.log
...
if [ -f $(pwd)/out/arch/arm64/boot/Image ]
	...
	zip -r9 Lemper-${TANGGAL}.zip ./* -x .git .gitignore ./*.zip
