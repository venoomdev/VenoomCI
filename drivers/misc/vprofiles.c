...
#include <linux/vprofiles.h>
#include <linux/delay.h>
#ifdef CONFIG_AUTO_VPROFILES_MSM_DRM
#include <linux/msm_drm_notify.h>
#elif defined(CONFIG_AUTO_VPROFILES_FB)
#include <linux/fb.h>
#endif

#if defined(CONFIG_AUTO_VPROFILES_MSM_DRM) || defined(CONFIG_AUTO_VPROFILES_FB)
...
static bool auto_vprofiles = true;
module_param(auto_vprofiles, bool, 0664);
module_param(mode, uint, 0664);

inline void vprofiles_set_mode_rollback(unsigned int level, unsigned int duration_ms)
{
	if (level && duration_ms && auto_vprofiles) {
		rollback_mode = mode;
		mode = level;
		msleep(duration_ms);
		mode = rollback_mode;
...
inline void vprofiles_set_mode(unsigned int level)
{
	if (level && auto_vprofiles)
		mode = level;
}
				...
				unsigned long event, void *data)
{
	...
	if (!screen_on && auto_vprofiles) {
		set_mode = mode;
		vprofiles_set_mode(1);
	}
	else if(screen_on && auto_vprofiles) {
		vprofiles_set_mode(set_mode);
	}
...
static int  __init vprofiles_init(void)
{
//	set_mode = mode;
#ifdef CONFIG_AUTO_VPROFILES_MSM_DRM
...
late_initcall(vprofiles_init);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("pocongtobat");
